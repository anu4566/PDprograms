...
{ int i;
  ...
  if (!(*m_fork_is_active))
    error_exit("must have multiple processes in execution, when m_sync is called");
  if (*m_single_is_active) error_exit("must not call m_sync, when m_single is in execution");
  if (*m_lock_is_active) error_exit("must not call m_sync, when m_lock is called");
  p(m_sync_mutex);
  if (*last_sync != 1) {
